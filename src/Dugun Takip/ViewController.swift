...
// ===== Uygulama İçi Satın Alma (StoreKit 2) — Genix Menü =====
extension ViewController {
    func handleIAP(_ message: WKScriptMessage) {
        ...
        let productId = (body["productId"] as? String) ?? "com.genixsoft.menu.pro.annual"
        switch action {
...
// ===== Face ID / Touch ID / Ekran Kilidi (App Lock) — Genix Menü =====
...
extension ViewController {
    ...
    func genixShowLock() {
        ...
        overlay.backgroundColor = UIColor(red: 0.09, green: 0.05, blue: 0.02, alpha: 1.0)
        ...
        icon.tintColor = UIColor(red: 0.98, green: 0.57, blue: 0.24, alpha: 1.0)
        ...
        btn.backgroundColor = UIColor(red: 0.85, green: 0.38, blue: 0.05, alpha: 1.0)
    ...
    private func genixRunBiometric() {
        ...
        if ctx.canEvaluatePolicy(.deviceOwnerAuthentication, error: &err) {
            ctx.evaluatePolicy(.deviceOwnerAuthentication,
                               localizedReason: "Genix Menü'yü açmak için kimliğinizi doğrulayın") { ok, _ in
                DispatchQueue.main.async {
